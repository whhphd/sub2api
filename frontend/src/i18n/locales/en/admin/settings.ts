...
"turnStateHint": "Applies globally to OpenAI OAuth HTTP/SSE. Collects candidates from natural responses and hunter probes; hunted models inject a live matching candidate whenever one is available, while other models wait for a non-baseline session shape. Exhaustion forwards normally by default; the optional hunter scheduling hold pauses accounts without a usable candidate and fails over until a matching candidate is obtained. WebSockets remain unchanged; decisions have independent logs. Experimental; quality improvement is not guaranteed.",
