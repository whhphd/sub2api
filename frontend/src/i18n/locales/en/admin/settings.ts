export default {
    settings: {
      ...
      gatewayForwarding: {
        ...
        openaiOAuthDefaultCodexFingerprintHint: 'Enabled by default. Newly created or imported OpenAI OAuth accounts without an explicit mode use the selected convergence mode. Existing accounts are unchanged.',
        openaiOAuthCodexFingerprintModeOff: 'Off',
        openaiOAuthCodexFingerprintModeDevice: 'Device only',
        openaiOAuthCodexFingerprintModeSession: 'Device + session',
        openaiOAuthCodexFingerprintModeFull: 'Full convergence',
        openaiOAuthNewAccountProxyPool: 'Random proxy pool for new OpenAI OAuth accounts',
        openaiOAuthNewAccountProxyPoolHint: 'When enabled, newly created or imported OpenAI OAuth accounts without an explicit proxy randomly select one of the active proxies below. Existing accounts are unchanged.',
        openaiOAuthNewAccountProxyPoolSearch: 'Search name, host/IP, port, or protocol',
        openaiOAuthNewAccountProxyPoolSelected: '{selected} of {total} selected',
        openaiOAuthNewAccountProxyPoolSelectResults: 'Select results',
        openaiOAuthNewAccountProxyPoolClear: 'Clear all',
        openaiOAuthNewAccountProxyPoolEmpty: 'No active proxies in IP Management',
        openaiOAuthNewAccountProxyPoolNoResults: 'No matching proxies',
        openaiOAuthNewAccountProxyPoolUnavailable: '{count} selected proxies are inactive, expired, or deleted and will be ignored when accounts are added.',
        openaiOAuthNewAccountProxyPoolClearUnavailable: 'Remove unavailable',
        openaiOAuthNewAccountProxyPoolRequired: 'Select at least one proxy when the random proxy pool is enabled.',
        openaiOAuthNewAccountProxyPoolLoadFailed: 'Failed to load proxies from IP Management',
  ...
  "description": "One policy for all OpenAI OAuth accounts, including new accounts. Probes obtain 292/332 candidates through selected proxies; probe accounting is enabled by default and degraded scheduling hold is off by default.",
  ...
  "autoModels": "Choose models automatically from real traffic (images excluded)",
  ...
  "rotatingProxies": "Explicitly mark proxies that rotate exit on every connection",
  ...
  "gap_seconds": "Probe interval per account (seconds)",
  ...
  "accountIntervalHint": "Accounts probe independently in parallel. Each account waits this interval after an attempt before selecting its next model. Retry, cooldown, quota and hourly budgets still apply. Exempt models are not actively probed.",
  "effort": "Probe reasoning effort",
  "released": "Settings saved; released {count} candidate holds and refreshed scheduling caches",
  "usageHint": "With a dedicated API key, each HTTP 200 probe uses standard billing and is labeled Hunter probe. Input tokens are estimated locally, output is recorded as zero. Charges apply to the selected key owner or subscription.",
  "usageMissing": "No billing API key configured: usage bills are skipped, while the hunter and diagnostic logs remain active.",
  ...
  "holdExclusionsHint": "Exact upstream model names. Forward without candidates, inject when available, and continue natural collection without active probes. Saving releases their candidate holds immediately. Real quota limits and credential faults remain enforced.",
...
"turnStateHint": "Applies globally to OpenAI OAuth HTTP/SSE. Collects candidates from natural responses and hunter probes; hunted models inject a live matching candidate whenever one is available, while other models wait for a non-baseline session shape. Exhaustion forwards normally by default; the optional hunter scheduling hold pauses only account/model pairs without a usable candidate and fails over until a matching candidate is obtained. WebSockets remain unchanged; decisions have independent logs. Experimental; quality improvement is not guaranteed.",
...
"turnStateSaveFailed": "Failed to save Turn-State settings",
        ...
        safeRetryEnabledHint: 'Only affects OpenAI OAuth Responses streams. Retries and account switching remain disabled after real output starts.',
        safeRetrySaved: 'OpenAI OAuth safe streaming overload retry settings saved',
        safeRetrySaveFailed: 'Failed to save OpenAI OAuth safe streaming overload retry settings',
        rateLimitSameAccountRetryTitle: 'OpenAI OAuth Short Rate-Limit Same-Account Retry',
        rateLimitSameAccountRetryDescription: 'Retry short Rate limit exceeded responses on the current OAuth account first to reduce cache misses caused by switching accounts.',
        rateLimitSameAccountRetryEnabled: 'Enable short rate-limit same-account retry',
        rateLimitSameAccountRetryEnabledHint: 'Retry up to 5 times by default, then use the existing account-switch flow; usage-exhaustion 429 responses are unaffected.',
        rateLimitSameAccountRetrySaved: 'OpenAI OAuth short rate-limit same-account retry settings saved',
        rateLimitSameAccountRetrySaveFailed: 'Failed to save OpenAI OAuth short rate-limit same-account retry settings',
        fingerprintEnhancementTitle: 'Codex fingerprint enhancement',
        fingerprintEnhancementDescription: 'Align OpenAI OAuth device identity, request format and egress environment. Uses device mode while enabled and restores the saved account mode when disabled.',
        fingerprintEnhancementEnabled: 'Enable Codex fingerprint enhancement',
        fingerprintEnhancementEnabledHint: 'Saving as enabled turns off short rate-limit proxy rotation. Same-account retries and proxy fault recovery continue. Disabling enhancement does not enable proxy rotation.',
        fingerprintEnhancementSaved: 'Codex fingerprint enhancement settings saved',
        fingerprintEnhancementSaveFailed: 'Failed to save Codex fingerprint enhancement settings',
        fingerprintEnhancementAccountNotice: 'Global Codex fingerprint enhancement is active. Device mode currently applies; the saved account mode below resumes when global enhancement is disabled.',
        proxyRotationTitle: 'OpenAI OAuth Short Rate-Limit Proxy Rotation',
        proxyRotationDescription: 'When a short Rate limit exceeded response is detected, randomly switch the OAuth account to an active proxy from IP management.',
        proxyRotationEnabled: 'Enable short rate-limit proxy rotation',
        proxyRotationEnabledHint: 'Only applies to OpenAI OAuth accounts; usage-exhaustion 429 responses do not rotate the proxy. If no replacement is available, the current proxy is kept. Saving as enabled turns off Codex fingerprint enhancement.',
        proxyRotationSaved: 'OpenAI OAuth short rate-limit proxy rotation settings saved',
        proxyRotationSaveFailed: 'Failed to save OpenAI OAuth short rate-limit proxy rotation settings',
        autoResetCreditGlobalTitle: 'OpenAI OAuth Global Automatic Reset-Credit Use',
        autoResetCreditGlobalDescription: 'Enable automatic reset-credit use and the one-minute compensation scan for all OpenAI OAuth parent accounts.',
        autoResetCreditGlobalEnabled: 'Enable automatic reset-credit use globally',
        autoResetCreditGlobalEnabledHint: 'Applies to every OpenAI OAuth parent account. Account-specific 5h/7d thresholds are retained; unconfigured thresholds default to 100%. Disabling restores account-level switches.',
        autoResetCreditGlobalSaved: 'OpenAI OAuth global automatic reset-credit setting saved',
        autoResetCreditGlobalSaveFailed: 'Failed to save the OpenAI OAuth global automatic reset-credit setting',
        grokForbiddenRetryTitle: 'Grok OAuth 403 Same-Account Retry',
        grokForbiddenRetryDescription: 'Control retry and scheduling-state handling for non-content-policy 403 responses from Grok OAuth accounts.',
        grokForbiddenRetryEnabled: 'Enable Grok OAuth 403 same-account retry',
        grokForbiddenRetryEnabledHint: 'Retry the current account 3 times without marking it temporarily unschedulable during or after exhaustion, then allow the existing account failover flow.',
        grokForbiddenRetrySaved: 'Grok OAuth 403 same-account retry settings saved',
        grokForbiddenRetrySaveFailed: 'Failed to save Grok OAuth 403 same-account retry settings',
        planGatedCooldownTitle: 'OpenAI OAuth Plan-Gated Model Cooldown',
        planGatedCooldownDescription: 'Control whether a Codex plan-gated model 400 temporarily pauses that model for an OpenAI OAuth account.',
        planGatedCooldownEnabled: 'Enable plan-gated model cooldown',
        planGatedCooldownEnabledHint: 'When disabled, skip the temporary model cooldown and use the ordinary failover and retry flow; genuine 404 model-not-found handling is unchanged.',
        planGatedCooldownSaved: 'OpenAI OAuth plan-gated model cooldown settings saved',
        planGatedCooldownSaveFailed: 'Failed to save OpenAI OAuth plan-gated model cooldown settings'
      },
      streamTimeout: {
