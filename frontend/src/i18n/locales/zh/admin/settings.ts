export default {
  ...
  "holdHint": "开启后，猎手管理的模型缺少有效基线候选时暂停该账号并换号；没有其他账号时返回 503。取得对应模型候选后自动恢复；关闭并保存时立即解除本功能暂停并刷新缓存，不清除额度限流和其他故障。WS 保持透传。",
  "usageApiKey": "探测记账 API Key ID（0=不记账）",
  "usageAccounting": "启用探测用量记账",
  "holdWhenDegraded": "降智时暂停账号调度",
...
"turnStateHint": "全局覆盖 OpenAI OAuth 的 HTTP/SSE 请求。从自然响应和猎手探测积累候选；猎手管理的模型有可用候选时直接按账号和模型注入，其他模型仅在会话出现非基线形态后使用候选。候选耗尽默认继续转发；开启猎手的暂停调度开关后，缺少可用候选的账号会暂停并尝试换号，补到对应模型候选后恢复。WS 保持透传，决策记录独立保存。该功能为实验性机制，不保证改善模型质量。",
