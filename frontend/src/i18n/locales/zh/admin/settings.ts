export default {
    settings: {
      ...
      features: {
        channelMonitor: {
          title: '渠道监控',
          description: 'V1 仅运行主动探测；V2 同时运行主动探测与被动用量监控。关闭后两类后台任务均停止。',
          configureLink: '前往 渠道管理 > 渠道监控 配置监控项',
          enabled: '启用渠道监控',
          enabledHint: '关闭后 V1 调度器与 V2 聚合均停止；已有配置与历史保留。',
          mode: '监控模式',
          modeHint: '默认 V1（仅主动探测）。切换到 V2 后保留主动探测，并增加真实网关流量聚合。',
          modeV2: 'V2 综合监控',
          modeV1: 'V1 主动探测',
          modeV2Hint: '基于真实网关流量聚合健康指标，同时继续按已启用监测项定时发起 V1 主动探测。',
          modeV1Hint: '默认模式：按配置的渠道监控项定时发起上游健康检查（产生探测流量）。',
          defaultInterval: '默认检测间隔（秒）',
          defaultIntervalHint: '新建 V1 主动探测项时的默认值，可被单个监测项覆盖。V1/V2 模式下均生效，范围 15 – 3600 秒。',
      ...
      gatewayForwarding: {
        ...
        openaiOAuthDefaultCodexFingerprintHint: '默认开启。新建或导入的 OpenAI OAuth 账号若未显式选择档位，将使用右侧所选的收敛模式；只影响新增账号，不修改存量账号。',
        openaiOAuthCodexFingerprintModeOff: '关闭',
        openaiOAuthCodexFingerprintModeDevice: '仅设备级',
        openaiOAuthCodexFingerprintModeSession: '设备+会话',
        openaiOAuthCodexFingerprintModeFull: '完全收敛',
        openaiOAuthNewAccountProxyPool: '新 OpenAI OAuth 账号随机代理池',
        openaiOAuthNewAccountProxyPoolHint: '开启后，新建或导入且未显式指定代理的 OpenAI OAuth 账号，会从以下启用代理中随机选择一个；只影响新增账号。',
        openaiOAuthNewAccountProxyPoolSearch: '搜索名称、主机/IP、端口或协议',
        openaiOAuthNewAccountProxyPoolSelected: '已选 {selected} / {total}',
        openaiOAuthNewAccountProxyPoolSelectResults: '全选当前结果',
        openaiOAuthNewAccountProxyPoolClear: '清除全部',
        openaiOAuthNewAccountProxyPoolEmpty: 'IP 管理中暂无启用代理',
        openaiOAuthNewAccountProxyPoolNoResults: '没有匹配的代理',
        openaiOAuthNewAccountProxyPoolUnavailable: '已选项中有 {count} 个代理已停用、过期或删除，入池时会自动忽略。',
        openaiOAuthNewAccountProxyPoolClearUnavailable: '清除失效项',
        openaiOAuthNewAccountProxyPoolRequired: '开启随机代理池时，请至少选择一个代理。',
        openaiOAuthNewAccountProxyPoolLoadFailed: '加载 IP 管理代理列表失败',
  ...
  "description": "统一管理所有 OpenAI OAuth 账号（含后续新增账号）。通过指定代理主动补充 292/332 候选；探测记账默认开启，降智暂停默认关闭。",
  "enabled": "启用全局猎手",
  "needsAuto": "运行需要同时开启 Turn-State 自动接管。",
  "models": "目标上游模型（最多 8 个）",
  "modelsPlaceholder": "模型名，用逗号分隔",
  "autoModels": "按真实请求自动选择模型（排除图片模型）",
  ...
  "rotatingProxies": "显式标记每次连接都会换出口的代理",
  "missingProxies": "已选代理不存在：{ids}，请重新选择。",
  "max_per_hour": "全站每小时探测上限",
  "per_account_max_per_hour": "单账号每小时探测上限",
  "gap_seconds": "每账号探测间隔（秒）",
  "lead_minutes": "候选到期前开窗（分钟）",
  "retry_minutes": "固定出口再次检查间隔（分钟）",
  "idle_minutes": "模型空闲门槛（分钟，-1 不限制）",
  "accountIntervalHint": "各账号独立并行；同账号一次探测结束后等待此间隔，再轮询下一个缺票模型。重试、错误退避、额度检查和小时预算仍生效。免暂停模型不主动打票。",
  "effort": "探测思考强度",
  "released": "配置已保存，已解除 {count} 个账号的候选暂停并刷新调度缓存",
  "usageHint": "填写专用 API Key 后，每次 HTTP 200 探测按标准计费记录为猎手探测。输入 Token 为本地估算，输出记为 0；会扣除所选 Key 所属用户余额或订阅额度。",
  "usageMissing": "未配置记账 API Key，暂不写入用量账单；猎手仍正常运行并记录诊断日志。",
  "holdHint": "开启后，猎手管理的模型缺少有效基线候选时仅暂停该账号对应模型并换号；没有其他账号时返回 503。取得对应模型候选后自动恢复；关闭并保存时立即解除本功能暂停并刷新缓存，不清除额度限流和其他故障。WS 保持透传。",
  "usageApiKey": "探测记账 API Key ID（0=不记账）",
  "usageAccounting": "启用探测用量记账",
  "holdExclusions": "免暂停模型名单（逗号分隔）",
  "holdExclusionsHint": "填写实际上游模型名，精确匹配。名单内模型缺候选时正常转发，有候选仍注入，不再主动打票，仍从自然响应采集。保存立即解除这些模型的候选暂停；真实额度限流和凭据故障不受影响。",
  "holdWhenDegraded": "缺候选时暂停对应模型调度",
...
"turnStateHint": "全局覆盖 OpenAI OAuth 的 HTTP/SSE 请求。从自然响应和猎手探测积累候选；猎手管理的模型有可用候选时直接按账号和模型注入，其他模型仅在会话出现非基线形态后使用候选。候选耗尽默认继续转发；开启猎手的暂停调度开关后，仅暂停缺少候选的账号与模型组合并尝试换号，补到对应模型候选后恢复。WS 保持透传，决策记录独立保存。该功能为实验性机制，不保证改善模型质量。",
...
"turnStateSaveFailed": "保存 Turn-State 设置失败",
        ...
        safeRetryEnabledHint: '仅影响 OpenAI OAuth Responses 流；已有真实内容输出后仍不会重试或切换账号。',
        safeRetrySaved: 'OpenAI OAuth 流式过载安全重试设置保存成功',
        safeRetrySaveFailed: '保存 OpenAI OAuth 流式过载安全重试设置失败',
        rateLimitSameAccountRetryTitle: 'OpenAI OAuth 短期限流原地重试',
        rateLimitSameAccountRetryDescription: '短期 Rate limit exceeded 时优先在当前 OAuth 账号上重试，减少切号造成的缓存失配。',
        rateLimitSameAccountRetryEnabled: '启用短期限流原地重试',
        rateLimitSameAccountRetryEnabledHint: '默认最多原地重试 5 次，失败后才按现有逻辑切号；额度耗尽类 429 不受此开关影响。',
        rateLimitSameAccountRetrySaved: 'OpenAI OAuth 短期限流原地重试设置保存成功',
        rateLimitSameAccountRetrySaveFailed: '保存 OpenAI OAuth 短期限流原地重试设置失败',
        fingerprintEnhancementTitle: 'Codex 指纹增强',
        fingerprintEnhancementDescription: '统一 OpenAI OAuth 的设备身份、请求格式与出口环境信息。开启后按设备模式运行，关闭后恢复账号保存的指纹模式。',
        fingerprintEnhancementEnabled: '启用 Codex 指纹增强',
        fingerprintEnhancementEnabledHint: '保存为开启时自动关闭短期限流代理切换；同账号重试和代理故障恢复继续运行。关闭本功能不会自动打开代理切换。',
        fingerprintEnhancementSaved: 'Codex 指纹增强设置保存成功',
        fingerprintEnhancementSaveFailed: '保存 Codex 指纹增强设置失败',
        fingerprintEnhancementAccountNotice: '全局 Codex 指纹增强已开启，当前按设备模式运行。下方是账号保存的模式，关闭全局增强后恢复生效。',
        proxyRotationTitle: 'OpenAI OAuth 短期限流代理切换',
        proxyRotationDescription: '检测到短时 Rate limit exceeded 时，从 IP 管理的活跃代理中随机更换当前 OAuth 账号代理。',
        proxyRotationEnabled: '启用短期限流代理切换',
        proxyRotationEnabledHint: '仅适用于 OpenAI OAuth 账号；用量耗尽类 429 不触发。当前代理没有可替换候选时保持原代理。保存为开启时会自动关闭 Codex 指纹增强。',
        proxyRotationSaved: 'OpenAI OAuth 短期限流代理切换设置保存成功',
        proxyRotationSaveFailed: '保存 OpenAI OAuth 短期限流代理切换设置失败',
        autoResetCreditGlobalTitle: 'OpenAI OAuth 全局自动使用重置卡',
        autoResetCreditGlobalDescription: '全局启用 OpenAI OAuth 母账号的自动用卡和每分钟补偿扫描。',
        autoResetCreditGlobalEnabled: '全局启用自动使用重置卡',
        autoResetCreditGlobalEnabledHint: '开启后所有 OpenAI OAuth 母账号均生效；优先使用账号已配置的 5h/7d 阈值，未配置时均为 100%。关闭后恢复账号级开关。',
        autoResetCreditGlobalSaved: 'OpenAI OAuth 全局自动用卡设置保存成功',
        autoResetCreditGlobalSaveFailed: '保存 OpenAI OAuth 全局自动用卡设置失败',
        grokForbiddenRetryTitle: 'Grok OAuth 403 原地重试',
        grokForbiddenRetryDescription: '控制 Grok OAuth 账号收到非内容审核类 403 后的重试和调度状态处理。',
        grokForbiddenRetryEnabled: '启用 Grok OAuth 403 原地重试',
        grokForbiddenRetryEnabledHint: '开启后在当前账号原地重试 3 次，重试期间及用尽后均不置为临时不可调度；随后仍可按现有逻辑切换账号。',
        grokForbiddenRetrySaved: 'Grok OAuth 403 原地重试设置保存成功',
        grokForbiddenRetrySaveFailed: '保存 Grok OAuth 403 原地重试设置失败',
        planGatedCooldownTitle: 'OpenAI OAuth 计划模型暂停',
        planGatedCooldownDescription: '控制 OpenAI OAuth 账号收到 Codex 计划不支持模型的 400 后是否暂停该模型调度。',
        planGatedCooldownEnabled: '启用计划模型暂停',
        planGatedCooldownEnabledHint: '关闭后不写入临时模型暂停，直接沿用普通失败转账号和重试逻辑；真正的 404 模型不存在处理不变。',
        planGatedCooldownSaved: 'OpenAI OAuth 计划模型暂停设置保存成功',
        planGatedCooldownSaveFailed: '保存 OpenAI OAuth 计划模型暂停设置失败'
      },
      streamTimeout: {
        title: '流超时处理',
