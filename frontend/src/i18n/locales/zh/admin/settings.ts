export default {
    settings: {
      ...
      openaiOauthRuntime: {
        injectionTitle: 'OpenAI OAuth 工具调用注入',
        injectionDescription: '统一控制所有 OpenAI OAuth 账号的空 exec 工具调用注入。',
        injectionEnabled: '启用工具调用注入',
        injectionEnabledHint: '开启后，每条用户消息后都会注入一对固定的 custom_tool_call 与 custom_tool_call_output。',
        injectionSaved: 'OpenAI OAuth 工具调用注入设置保存成功',
        injectionSaveFailed: '保存 OpenAI OAuth 工具调用注入设置失败',
        ...
        dynamic429Title: 'OpenAI OAuth 429 动态调度',
        dynamic429Description: '将 OpenAI OAuth 类 429 作为统计信号，按窗口内 429 比例决定是否暂停调度。',
        dynamic429Enabled: '启用动态调度',
        dynamic429EnabledHint: '首个 429 后开始统计 429 与成功/其他错误；达到阈值后调用统一限流状态暂停账号。',
        windowSeconds: '统计窗口（秒）',
        windowSecondsHint: '从首个 429 开始计数的窗口时长（60–3600 秒）',
        minimumSamples: '最小样本数',
        minimumSamplesHint: '达到该样本数后才判断比例（2–10000）',
        minimum429Count: '最少 429 次数',
        minimum429CountHint: '429 次数至少达到该值才可能暂停（1–最小样本数）',
        ratioThreshold: '429 比例阈值',
        ratioThresholdHint: '窗口内 429 / 总样本比例，范围 0.01–1',
        pauseMode: '暂停时长',
        pauseModeUpstreamReset: '按原有暂停时长逻辑处理',
        pauseModeUpstreamResetHint: '优先使用上游额度重置时间，其次使用系统 429 默认回避配置。',
        pauseModeFixed: '固定时长',
        fixedPauseSeconds: '固定暂停时长（秒）',
        fixedPauseSecondsHint: '达到阈值后暂停调度的时长（1–7200 秒）',
        dynamic429Saved: 'OpenAI OAuth 429 动态调度设置保存成功',
        dynamic429SaveFailed: '保存 OpenAI OAuth 429 动态调度设置失败'
      },
      streamTimeout: {
        title: '流超时处理',
