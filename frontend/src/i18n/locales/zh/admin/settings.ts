...
"turnStateHint": "全局覆盖 OpenAI OAuth 的 HTTP/SSE 请求。从自然响应和猎手探测积累候选；猎手管理的模型有可用候选时直接按账号和模型注入，其他模型仅在会话出现非基线形态后使用候选。候选耗尽不停止调度。WS 保持透传，决策记录独立保存。该功能为实验性机制，不保证改善模型质量。",
