export default {
    accounts: {
      ...
      recentRequests: {
        summary: "最近 {count} 条请求记录",
        viewDetails: "查看 {time} 的请求",
        success: "成功请求",
        error: "错误记录",
        reason: "错误原因",
        unknownError: "暂无错误详情",
        ...
        stale: "刷新失败 · 显示上次记录",
        empty: "24 小时内无记录",
        hint: "最近 24 小时内最多 10 条记录，右侧为最新；绿色为成功，红色为错误。页面可见时每 5 秒读取本地日志。重试错误可能与最终成功共存，不代表实时并发或成功率。",
      },
      columns: {
        recentRequests: '最近请求状态',
        name: '名称',
        id: '账号ID',
        platformType: '平台/类型',
        platform: '平台',
      ...
      openai: {
        ...
        codexFingerprintModeDesc: '多人共享同一 OAuth 账号时，将各用户的设备/会话标识收敛为账号级稳定值，减少上游可见的设备数和会话数。账号未显式选择时遵循系统默认；账号级选择优先。',
	  ...
	  autoResetCredit: {
	    title: '自动使用重置卡',
	    hint: '仅在实际用量达到阈值时使用最早到期的可用卡；默认关闭。明确无卡时继续由上游 429 设置限流，限流期间仍会定期查卡并自动恢复。',
