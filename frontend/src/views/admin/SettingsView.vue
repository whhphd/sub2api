    ...
    <div class="mx-auto max-w-6xl space-y-6">
      ...
      <form v-else @submit.prevent="saveSettings" class="space-y-6" novalidate>
        ...
        <div v-show="activeTab === 'gateway'" class="space-y-6">
          ...
          <div
            class="card"
            data-testid="grok-oauth-403-same-account-retry-card"
          >
            <div
              class="border-b border-gray-100 px-6 py-4 dark:border-dark-700"
            >
              <h2 class="text-lg font-semibold text-gray-900 dark:text-white">
                {{
                  t(
                    "admin.settings.openaiOauthRuntime.grokForbiddenRetryTitle",
                  )
                }}
              </h2>
              <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                {{
                  t(
                    "admin.settings.openaiOauthRuntime.grokForbiddenRetryDescription",
                  )
                }}
              </p>
            </div>
              ...
              <template v-else>
                <div class="flex items-center justify-between gap-6">
                  <div>
                    <label class="font-medium text-gray-900 dark:text-white">
                      {{
                        t(
                          "admin.settings.openaiOauthRuntime.grokForbiddenRetryEnabled",
                        )
                      }}
                    </label>
                    <p class="text-sm text-gray-500 dark:text-gray-400">
                      {{
                        t(
                          "admin.settings.openaiOauthRuntime.grokForbiddenRetryEnabledHint",
                        )
                      }}
                    </p>
                  </div>
                  <Toggle
                    v-model="openAIOAuthRuntimeForm.grok_oauth_forbidden_same_account_retry_enabled"
                    :disabled="grokOAuthForbiddenSameAccountRetrySaving"
                ...
                >
                  <button
                    ...
                    class="btn btn-primary btn-sm"
                    :disabled="grokOAuthForbiddenSameAccountRetrySaving"
                    data-testid="grok-oauth-403-same-account-retry-save"
                    @click="saveGrokOAuthForbiddenSameAccountRetrySettings"
                  >
                    {{
                      grokOAuthForbiddenSameAccountRetrySaving
                        ? t("common.saving")
                        : t("common.save")
                    }}
                  </button>
          ...
          <!-- OpenAI OAuth Safe Stream Retry -->
          <div class="card" data-testid="openai-oauth-safe-retry-card">
            <div
              class="border-b border-gray-100 px-6 py-4 dark:border-dark-700"
            >
              <h2 class="text-lg font-semibold text-gray-900 dark:text-white">
                {{ t("admin.settings.openaiOauthRuntime.safeRetryTitle") }}
              </h2>
              <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                {{ t("admin.settings.openaiOauthRuntime.safeRetryDescription") }}
              </p>
            </div>
            <div class="space-y-5 p-6">
              <div
              ...
              <template v-else>
                <div class="flex items-center justify-between gap-6">
                  <div>
                    <label class="font-medium text-gray-900 dark:text-white">
                      {{ t("admin.settings.openaiOauthRuntime.safeRetryEnabled") }}
                    </label>
                    <p class="text-sm text-gray-500 dark:text-gray-400">
                      {{ t("admin.settings.openaiOauthRuntime.safeRetryEnabledHint") }}
                    </p>
                  </div>
                  <Toggle
                    v-model="openAIOAuthRuntimeForm.safe_pre_output_overload_retry_enabled"
                    :disabled="openAIOAuthSafeRetrySaving"
                    data-testid="openai-oauth-safe-retry-toggle"
                  />
                </div>
                <div
                  class="flex justify-end border-t border-gray-100 pt-4 dark:border-dark-700"
                >
                  <button
                    ...
                    class="btn btn-primary btn-sm"
                    :disabled="openAIOAuthSafeRetrySaving"
                    data-testid="openai-oauth-safe-retry-save"
...
async function saveOpenAIOAuthSafeRetrySettings() {
  openAIOAuthSafeRetrySaving.value = true;
  try {
    ...
    appStore.showSuccess(t("admin.settings.openaiOauthRuntime.safeRetrySaved"));
  } catch (error: unknown) {
    appStore.showError(
      extractApiErrorMessage(
        error,
        t("admin.settings.openaiOauthRuntime.safeRetrySaveFailed"),
      ),
    );
  } finally {
...
async function saveGrokOAuthForbiddenSameAccountRetrySettings() {
  grokOAuthForbiddenSameAccountRetrySaving.value = true;
  try {
    const updated = await adminAPI.settings.updateOpenAIOAuthRuntimeSettings({
    ...
    openAIOAuthRuntimeForm.grok_oauth_forbidden_same_account_retry_enabled =
      updated.grok_oauth_forbidden_same_account_retry_enabled;
    appStore.showSuccess(
      t("admin.settings.openaiOauthRuntime.grokForbiddenRetrySaved"),
    );
  } catch (error: unknown) {
    appStore.showError(
      extractApiErrorMessage(
        error,
        t("admin.settings.openaiOauthRuntime.grokForbiddenRetrySaveFailed"),
      ),
    );
  } finally {
    grokOAuthForbiddenSameAccountRetrySaving.value = false;
  }
}
