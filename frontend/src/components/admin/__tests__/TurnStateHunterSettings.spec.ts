...
const config = { hold_excluded_models: ['gpt-5.6-terra'] as string[], auto_models: false, rotating_proxy_ids: [] as number[], hold_when_degraded: false, usage_accounting_enabled: true, usage_api_key_id: 0, enabled: false, models: ['gpt-test'], proxy_ids: [2], max_per_hour: 300, per_account_max_per_hour: 30, gap_seconds: 20, lead_minutes: 10, retry_minutes: 10, idle_minutes: 60, reasoning_effort: 'high' }
...
it('preserves an explicitly empty exemption list', async()=>{
 get.mockResolvedValue({openai_oauth_turn_state_auto_enabled:true,openai_oauth_turn_state_hunter:{...config,hold_excluded_models:[]}})
 await render();expect((wrapper.get('[data-testid="hunter-hold-exclusions"]').element as HTMLInputElement).value).toBe('')
 await wrapper.get('[data-testid="hunter-save"]').trigger('click');await flushPromises();expect(save.mock.calls[0][0].openai_oauth_turn_state_hunter.hold_excluded_models).toEqual([])
})
