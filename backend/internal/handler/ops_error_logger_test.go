...
func TestIsCountTokensPathIncludesResponsesInputTokensAliases(t *testing.T) {
	for _, path := range []string{
		...
		require.True(t, isTokenCountRequestPath(path), "path=%s", path)
	}
	require.False(t, isTokenCountRequestPath("/v1/responses"), "normal Responses requests are not token-count probes")
}
