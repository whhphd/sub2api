...
func TestChannelMonitorV2ErrorAggregationCountsFinalUserErrorsOnly(t *testing.T) {
	...
	require.Contains(t, query, "relevant_errors")
	require.Contains(t, query, "union all")
	require.Contains(t, query, "join candidate_ids candidate on candidate.request_id = current_error.request_id")
	require.NotContains(t, query, "current_error.request_id in (select request_id from candidate_ids)")
