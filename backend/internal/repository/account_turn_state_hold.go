...
func turnStateHoldPolicyLocked(ctx context.Context, tx *dbent.Tx)(bool,error){
 rows,err:=tx.Client().QueryContext(ctx,"SELECT value FROM settings WHERE key=$1 FOR SHARE",service.SettingKeyOpenAIOAuthRuntimeSettings);if err!=nil{return false,err};defer rows.Close()
 if !rows.Next(){return false,rows.Err()};var raw string;if err=rows.Scan(&raw);err!=nil{return false,err}
 var p service.OpenAIOAuthRuntimeSettings;if err=json.Unmarshal([]byte(raw),&p);err!=nil{return false,err}
...
func(r *accountRepository) ReleaseTurnStateHoldsIfDisabled(ctx context.Context)(int,error){
 tx,err:=r.client.Tx(ctx);if err!=nil{return 0,err};defer func(){_ = tx.Rollback()}()
 enabled,err:=turnStateHoldPolicyLocked(ctx,tx);if err!=nil{return 0,err};if enabled{return 0,errors.New("hold was enabled concurrently; reload settings")}
 ...
 if err!=nil{return 0,err};n,err:=result.RowsAffected();if err!=nil{return 0,err};if err=tx.Commit();err!=nil{return 0,err}
 ...
 if r.schedulerCache!=nil {
  accounts,e:=r.ListByPlatform(ctx,service.PlatformOpenAI);if e!=nil{return int(n),e}
  for i:=range accounts {if accounts[i].Type!=service.AccountTypeOAuth {continue};if e=r.schedulerCache.SetAccount(ctx,&accounts[i]);e!=nil{return int(n),e}}
 }
 return int(n),nil
}
