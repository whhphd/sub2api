...
func (c *proxyHealthCache) GetProxyHealth(ctx context.Context, proxyID int64) (*service.ProxyHealthState, error) {
	...
	state.ConsecutiveFailures = parseRedisInt(values[0])
...
func (c *proxyHealthCache) RecordProxyFailure(ctx context.Context, proxyID int64, now time.Time, window time.Duration, threshold int, cooldown time.Duration, failureClass, message string) (*service.ProxyHealthState, error) {
	...
	if !ok || len(values) < 3 { return nil, fmt.Errorf("unexpected proxy health script result %T", result) }
	...
	if open := parseRedisInt(values[2]); open > 0 { t := time.Unix(open, 0); state.OpenUntil = &t }
	return state, nil
}

func (c *proxyHealthCache) RecordProxySuccess(ctx context.Context, proxyID int64, now time.Time) error {
	if c == nil || c.rdb == nil || proxyID <= 0 { return nil }
	key := proxyHealthKey(proxyID)
	return c.rdb.TxPipelined(ctx, func(pipe redis.Pipeliner) error {
...
func redisString(value interface{}) string {
	switch v := value.(type) { case string: return v; case []byte: return string(v); default: return "" }
}

func parseRedisInt(value interface{}) int64 {
	return func() int64 { n, _ := strconv.ParseInt(redisString(value), 10, 64); if n != 0 { return n }; if f, ok := value.(int64); ok { return f }; return 0 }()
}

func parseRedisTime(value interface{}) time.Time {
	if n := parseRedisInt(value); n > 0 { return time.Unix(n, 0) }
	return time.Time{}
}
