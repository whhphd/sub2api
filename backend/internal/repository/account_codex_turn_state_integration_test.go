...
func TestHunterRuntimeSurvivesEditAndClearsOnIdentityChange(t *testing.T) {
	...
	require.NoError(t, err)
	require.NoError(t, repo.MutateCodexTurnState(ctx, a.ID, func(*service.Account) (map[string]any, error) {
		return map[string]any{service.CodexTurnStateHuntKey: map[string]any{"hour_count": 7}, service.CodexTurnStatePoolKey: "sealed"}, nil
	}))
	stale.Credentials["access_token"] = "refreshed"
	require.NoError(t, repo.Update(ctx, stale))
	latest, err := repo.GetByID(ctx, a.ID)
	require.NoError(t, err)
	require.Contains(t, latest.Extra, service.CodexTurnStateHuntKey)
	latest.Credentials["chatgpt_account_id"] = "new-owner"
	require.NoError(t, repo.Update(ctx, latest))
	latest, err = repo.GetByID(ctx, a.ID)
	require.NoError(t, err)
	require.NotContains(t, latest.Extra, service.CodexTurnStateHuntKey)
	require.NotContains(t, latest.Extra, service.CodexTurnStatePoolKey)
}
