...
func (s *OpenAIGatewayService) newGrokUpstreamFailoverError(
	...
	if retryable, retryDelay, retryDeadline, retryMax := grokSameAccountRetryMetadata(account, statusCode, responseBody); retryable {
		failoverErr.RetryableOnSameAccount = true
		failoverErr.SameAccountRetryDelay = retryDelay
		failoverErr.SameAccountRetryDeadline = retryDeadline
		failoverErr.SameAccountRetryMax = retryMax
		failoverErr.RequestScopedTransient = failoverErr.RequestScopedTransient || statusCode == http.StatusTooManyRequests
	}
	return failoverErr
}
