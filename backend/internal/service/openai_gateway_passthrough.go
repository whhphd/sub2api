...
func (s *OpenAIGatewayService) forwardOpenAIPassthrough(
	...
	if account != nil && account.UsesOpenAICodexProtocol() {
		...
		if isOpenAIResponsesCompactPath(c) && c.GetBool("codex_compact_identity_pending") {
	...
	if extractOpenAICodexTurnState(resp.Header) != "" {
		s.noteOpenAICodexTurnStateOrigin(c, account, resp.Header.Get(openAICodexTurnStateHeader))
	}
...
func (s *OpenAIGatewayService) buildUpstreamRequestOpenAIPassthrough(
	...
	if c != nil && c.Request != nil {
		for key, values := range c.Request.Header {
			...
			if !isOpenAIPassthroughAllowedRequestHeader(lower, allowTimeoutHeaders) || (codexEnhancementOnlyHeader(lower) && !codexDeviceWireProfileEnabled(c, account)) {
				continue
			}
...
func (s *OpenAIGatewayService) newOpenAIStreamFailoverErrorWithModel(
	...
	// Preserve retry metadata from both the OAuth 429 classifier and the
	// request-scoped capacity classifier.
...
func (s *OpenAIGatewayService) handleStreamingResponsePassthrough(
	...
	for documentScanner.Scan() {
		...
		if data, ok := extractOpenAISSEDataLine(line); ok {
				...
				if !outputStarted {
					...
					if shouldFailover {
						return resultWithUsage(),
							attachOpenAIOverloadDiagnostics(safePreOutputOverloadRetry, s.newOpenAIStreamFailoverErrorWithModel(c, account, true, upstreamRequestID, dataBytes, failedMessage, mappedModel, resp.Header), overloadTracker, false)
					}
			...
			visibleOutput := openAIStreamDataStartsVisibleOutput(trimmedData, eventType)
			if lineStartsClientOutput && !openAIStreamEventTypeIsTerminal(eventType) && visibleOutput {
				semanticOutputSeen = true
			}
			// Empty completed responses are silent upstream refusals and must fail over.
