...
func prepareCodexFingerprintExtraForUpdate(account *Account, extra map[string]any) map[string]any {
	...
	// Adapted from LuckyKuang/sub2api-plus 1f06d5871f: omission is not a
	// request to clear a saved mode. Explicit off/null remains an explicit edit.
	if _, provided := prepared[codexFingerprintModeExtraKey]; !provided {
		if mode, exists := account.Extra[codexFingerprintModeExtraKey]; exists {
			if prepared == nil {
				prepared = map[string]any{}
			}
			prepared[codexFingerprintModeExtraKey] = mode
		}
	}
...
func resolveCodexFingerprintMode(account *Account, enabled bool) (codexFingerprintMode, bool) {
	if account != nil && account.codexFingerprintEnhanced && account.IsOpenAIOAuth() {
...
func resolveCodexFingerprintIDsWithWindow(account *Account, clientSessionID string, mode codexFingerprintMode, clientWindowNumber string) *codexFingerprintIDs {
	return resolveCodexFingerprintIDsWithWindowSeed(account, clientSessionID, mode, clientWindowNumber, "")
}
func resolveCodexFingerprintIDsWithWindowSeed(account *Account, clientSessionID string, mode codexFingerprintMode, clientWindowNumber, seedOverride string) *codexFingerprintIDs {
...
func resolveCodexFingerprintIDsForRequest(c *gin.Context, account *Account, clientHeaders http.Header) *codexFingerprintIDs {
...
func resolveCodexFingerprintIDsWithBody(c *gin.Context, account *Account, clientHeaders http.Header, rawClientMetadata any) *codexFingerprintIDs {
	...
	mode, isDefault := resolveCodexFingerprintMode(account, account.codexUniqueFingerprint)
	...
	seedOverride := ""
	if isDefault {
		seedOverride = deriveAccountCodexFingerprintSeed(account)
		if persisted, ok := codexFingerprintSeed(account.Extra); ok {
			seedOverride = persisted
		}
	}
	ids := resolveCodexFingerprintIDsWithWindowSeed(account, clientSessionID, mode, clientWindowNumber, seedOverride)
	if ids == nil {
		return nil
	}
...
func resolveCodexFingerprintIDsFromRequest(account *Account, headers http.Header, enabled ...bool) *codexFingerprintIDs {
	if account == nil {
		return nil
	}
	copy := *account
	if len(enabled) > 0 {
		copy.codexUniqueFingerprint = enabled[0]
	}
	return resolveCodexFingerprintIDsForRequest(nil, &copy, headers)
}
