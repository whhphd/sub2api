...
func codexDeviceWireProfileEnabledFor(account, credentialAccount *Account) bool {
	return account != nil && activeCodexFingerprintMode(account) == codexFingerprintDevice &&
...
func applyCodexIdentityToWSPayload(c *gin.Context, account *Account, payload []byte) ([]byte, error) {
	...
	// Resolve legacy modes too: clearing staged IDs without restaging made the
	// second WS turn silently lose CallAI's existing device convergence.
