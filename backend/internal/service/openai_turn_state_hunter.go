...
func (s *OpenAITurnStateHunterService) huntOne(ctx context.Context, old *Account, cfg TurnStateHunterSettings) (spent, halt bool) {
 ...
 now := s.now()
 if st.BackoffVersion<2 {st.upgradeBackoff(now);if s.save(ctx,a,st)!=nil{return false,true}}
 if s.hunterAccountBlocked(ctx,a,now){s.gate(ctx,a,st,"account_unavailable_or_quota");return false,false}
	st.rollHour(now)
 ...
 for _,held:=range heldModels{if cfg.HoldWhenDegraded&&(cfg.AutoModels||cfg.hunts(held))&&!isOpenAIImageGenerationModel(held)&&!slices.Contains(configuredModels,held){configuredModels=append(configuredModels,held)}}
	active := false
  ...
  if st.ModelNext[model].After(now)||a.isRateLimitActiveForKey(model){continue}
		active = true
		expiry := time.Time{}
...
func (s *OpenAITurnStateHunterService) probeWithTransportRetry(ctx context.Context, account *Account, model string, cfg TurnStateHunterSettings, selected Proxy, exit string, st openAITurnStateHuntState) (spent, halt bool) {
	...
	for attempt := 1; attempt <= maxAttempts; attempt++ {
		...
		if s.hunterAccountBlocked(ctx,a,s.now()) || a.isRateLimitActiveForKey(model) {s.gate(ctx,a,st,"account_unavailable_or_quota");return spent,false}
