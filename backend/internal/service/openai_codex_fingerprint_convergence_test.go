...
func convTestAccount(enabled bool) *Account {
	...
	return &Account{codexPolicyPrepared: true, ID: 11, Platform: PlatformOpenAI, Type: AccountTypeOAuth,
...
func TestCodexFingerprintConvergence_WithDeviceMode(t *testing.T) {
	...
	require.True(t, applyCodexAccountIdentityClientMetadataMap(decoded, account, 77))
	c := newConvTestContext(t, rawBody)
	fp := resolveCodexFingerprintIDsForRequest(c, account, nil)
	require.NotNil(t, fp)
...
func TestCodexFingerprint_ImagesOAuthMatchesResponsesDeviceIdentity(t *testing.T) {
	...
	newAccount := func() *Account {
		...
		a.codexFingerprintEnhanced = true
	...
	_, _ = svcImg.forwardOpenAIImagesOAuth(withOpenAIImagesForceResponses(context.Background()), newCtx("/v1/images/generations"), newAccount(),
