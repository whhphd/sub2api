...
func TestHunterTerraDefaultHonorsExplicitEmpty(t *testing.T) {
	for _, raw := range []string{"", `{"openai_oauth_turn_state_hunter":{"enabled":true}}`} {
		p, e := parseOpenAIOAuthRuntimeSettings(raw)
		require.NoError(t, e)
		require.Equal(t, []string{"gpt-5.6-terra"}, p.TurnStateHunter.HoldExcludedModels)
	}
	p, e := parseOpenAIOAuthRuntimeSettings(`{"openai_oauth_turn_state_hunter":{"hold_excluded_models":[]}}`)
	require.NoError(t, e)
	require.Empty(t, p.TurnStateHunter.HoldExcludedModels)
}
