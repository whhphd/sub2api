		...
		if data, ok := extractOpenAISSEDataLine(line); ok {
			...
			if eventType == "response.failed" || eventType == "error" {
				...
				if !outputStarted {
					...
					if shouldFailover {
						sawFailedEvent = true
						streamEarlyErr = attachOpenAIOverloadDiagnostics(safePreOutputOverloadRetry, s.newOpenAIStreamFailoverErrorWithModel(c, account, false, upstreamRequestID, dataBytes, failedMessage, mappedModel, resp.Header), overloadTracker, false)
						return
					}
