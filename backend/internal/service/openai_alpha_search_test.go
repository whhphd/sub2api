...
func TestForwardAlphaSearchPATBackfillsMissingChatGPTAccountMetadata(t *testing.T) {
	...
	require.Equal(t, "true", upstream.lastReq.Header.Get("X-OpenAI-Fedramp"))
	require.NotContains(t, account.Credentials, "chatgpt_account_id", "request metadata backfill must not mutate the scheduler account")
	require.NotContains(t, account.Credentials, "chatgpt_user_id")
