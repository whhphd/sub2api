...
func DefaultTurnStateHunterSettings() TurnStateHunterSettings {
	return TurnStateHunterSettings{HoldExcludedModels: []string{}, Models: []string{}, ProxyIDs: []int64{}, RotatingProxyIDs: []int64{}, MaxPerHour: 300, PerAccountMaxPerHour: 30, GapSeconds: 20, LeadMinutes: 10, RetryMinutes: 10, IdleMinutes: 60, ReasoningEffort: "high", UsageAccountingEnabled: true}
}
...
func (c TurnStateHunterSettings) validate() error {
 if len(c.HoldExcludedModels)>64{return fmt.Errorf("at most 64 hold-exempt models")}
 exempt:=map[string]bool{}
 for _,m:=range c.HoldExcludedModels{if turnStateModel(m)==""||m!=turnStateModel(m)||exempt[m]{return fmt.Errorf("hold-exempt models must be unique normalized model names")};exempt[m]=true}
...
func(c TurnStateHunterSettings) HoldExempt(model string)bool{return slices.Contains(c.HoldExcludedModels,turnStateModel(model))}
