...
// Team-linked 402 fan-out is not part of this state-transition stub. Define
// the method explicitly so the embedded nil AccountRepository is never called
// when the manifest error path invokes the optional fan-out hook.
func (r *codexModelsAccountStateRepo) ListByPlatform(_ context.Context, _ string) ([]Account, error) {
	return nil, nil
}
