...
func TestStreamFailedEventCapacityShedRetriesOnSameAccount(t *testing.T) {
	...
	ordinary := []byte(`{"type":"response.failed","response":{"error":{"message":"An error occurred while processing your request."}}}`)
	require.False(t, isOpenAIUpstreamCapacityShedSignal(ordinary, "An error occurred while processing your request."))
...
func TestOpenAIStreamSafePreOutputMetadataBuffer(t *testing.T) {
	...
	// v0.1.179 now treats empty structural item events as pre-output even when
	// the extra overload guard is disabled.
	require.False(t, openAIStreamDataStartsClientOutputWithSafePreOutputRetry(metadata[0].data, metadata[0].typeName, false))
}
