...
func TestHunterAccountWorkersIndependentAndSequential(t *testing.T) {
	...
	case n := <-fastStarted:
		require.Equal(t, 1, n)
	case <-time.After(3 * time.Second):
		t.Fatal("slow account blocked other accounts")
	}
	...
	case n := <-fastStarted:
		require.Equal(t, 2, n)
	case <-time.After(3 * time.Second):
		t.Fatal("fast account did not resume independently")
	}
	cancel()
	select {
	case <-done:
	case <-time.After(3 * time.Second):
		t.Fatal("cancellation did not join all account workers")
	}
