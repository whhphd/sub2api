...
func (r *hunterAccounts) CompareAndSwapTurnStateHold(_ context.Context, e *Account, u *time.Time, reason string) (bool, error) {
 ...
 if model==reason||model==""{return false,errors.New("missing model")}
 current:=turnStateModelHolds(a);old:=turnStateModelHolds(e)
 if !reflect.DeepEqual(a.Credentials,e.Credentials)||!current[model].Equal(old[model]){return false,nil}
 if a.Extra==nil{a.Extra=map[string]any{}}
 raw,_:=a.Extra[CodexTurnStateModelHoldsKey].(map[string]any);if raw==nil{raw=map[string]any{}}
 if u==nil{delete(raw,model)}else{raw[model]=u.Format(time.RFC3339Nano)}
 a.Extra[CodexTurnStateModelHoldsKey]=raw
 if a.TempUnschedulableReason==reason&&e.TempUnschedulableReason==reason&&sameHoldTime(a.TempUnschedulableUntil,e.TempUnschedulableUntil){a.TempUnschedulableUntil=nil;a.TempUnschedulableReason=""}
	return true, nil
}
...
func TestHunterHoldPreventsNetworkAndAllowsFreshEcho(t *testing.T) {
	...
	require.NoError(t, err)
	req := stateBoundRequest(s.gateway, a, 1, "held", "gpt-test")
	resp, err := s.gateway.doOpenAIUpstream(req, "", a)
	require.Nil(t, resp)
	var fe *UpstreamFailoverError
	require.ErrorAs(t, err, &fe)
	require.Equal(t, OpenAITurnStateHoldReason, fe.Reason)
	echo := stateBoundRequest(s.gateway, a, 1, "echo", "gpt-test")
	at := turnStateAttemptFrom(echo.Context())
	at.Original = turnStateFernetBlob(time.Now(), 12)
	s.gateway.prepareTurnStateHTTP(echo)
	require.NoError(t, turnStateHoldError(echo))
}
